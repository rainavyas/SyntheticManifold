...
def fgsm_attack(x, y, model, criterion):
    '''
    x: torch.Tensor [Bx3]
    y: torch.Tensor [B]
    ...
    x_attacked = x + torch.sign(x.grad)
...
if __name__ == "__main__":
    ...
    commandLineParser.add_argument('OUT_DATA', type=str, help='path to save attacked test.npy file')
    ...
    commandLineParser.add_argument('--epsilon', type=float, default=10, help="attack size")
    ...
    # Attack
    if args.ATTACK == 'fgsm':
        x_attacked = fgsm_attack(x, y, model, criterion)
    
    # Save the attacked data
    np.save(args.OUT, x_attacked.cpu().detach().numpy())